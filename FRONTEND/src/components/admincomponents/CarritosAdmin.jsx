    ...
    const carritosFiltrados = carritos.filter(c =>
        ...
        (busquedaCliente
            ? ((c.cliente || "").toLowerCase().includes(busquedaCliente.toLowerCase()) ||
                (c.cliente_nombre || "").toLowerCase().includes(busquedaCliente.toLowerCase()))
            : true)
    );
                ...
                <tbody>
                    {carritosFiltrados.map(c => (
                        <tr key={c.id_carrito} className="carritos-admin-row">
                            ...
                            <td className="carritos-admin-td">{c.cliente || c.cliente_nombre || "-"}</td>
                            <td className="carritos-admin-td">{c.estado}</td>
                            <td className="carritos-admin-td">{c.fecha_creacion ? c.fecha_creacion.slice(0, 16).replace("T", " ") : "-"}</td>
                ...
                <div className="carritos-admin-detalle-modal">
                    <div>
                                        ...
                                        <tr key={idx}>
                                            ...
                                            <td>
                                                {item.imagen_producto ? (
                                                    <img
                                                        src={item.imagen_producto.startsWith("http") ? item.imagen_producto : `http://localhost:3000${item.imagen_producto}`}
                        ...
                        <h4 className="carritos-admin-detalle-total">
                            Total: {carritoDetalle.total_venta != null ? `$${carritoDetalle.total_venta}` : "0"}
                        </h4>
                        <button
